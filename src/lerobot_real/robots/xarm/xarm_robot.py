...
class XArmRobot(Robot, Thread):

    config_class = XArmRobotConfig
    name = "xArm Robot"

    def __init__(self, config: XArmRobotConfig, prefix=''):
        ...
        if self._dof == None or (not self._dof in (5,6,7)):
            raise ValueError(f"Please specify the correct DOF xArm robot!, got {self._dof}")
    ...
    def _robot_state_features(self)-> dict:
        if self._control_space == "joint":
            ...
            raise ValueError(f"Please check the given control space of xArm robot! got {self._control_space}")
    ...
    def connect(self, calibrate: bool = True) -> None:
        ...
        if not self._is_connected:
            print(f"xArm robot connection Failed, please check the hardware availability at ip: {self.config.robot_ip}")
    ...
    def configure(self) -> None:
        ...
        if err_warn[0] != 0:
            raise RuntimeError(f"Failed to set correct state to xArm robot! Controller Error code: {err_warn[0]} !")
    ...
    def get_observation(self) -> dict[str, np.ndarray]:
        ...
        if self._control_space == "joint":
            ...
            ValueError(f"Please check the given control space of xArm robot! got {self._control_space}")
    ...
    def run(self):
        ...
        print(f"xArm Robot ({self.config.robot_ip}) RT Report Thread starts!! =======")
        ...
        self._rt_report_normal = False
        print(f"xArm Robot ({self.config.robot_ip}) RT Report Thread Exit!! =======")
