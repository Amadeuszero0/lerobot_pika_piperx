...
from lerobot_real.teleoperators.xarm_mock_teleop import XArmMockTeleop
...
def _discard_current_episode(dataset, async_episode_saver=None):
    """Discard buffered frames while keeping the same episode index for a retry."""
    ...
    empty_episode_buffer = _create_empty_episode_buffer(
        dataset, episode_index, episode_buffer
    )
...
class AsyncEpisodeSaver:
    ...
    def __init__(self, dataset):
        ...
        self._thread = threading.Thread(target=self._run, name="lerobot-real-async-episode-saver", daemon=True)
        self._thread.start()
...
def record_loop(
    ...
    while control_time_s is None or timestamp < control_time_s:
        ...
        if command_poller is not None:
            _apply_record_command(events, command_poller())
        ...
        if isinstance(teleop, BaseTeleop) and teleop.has_pending_robot_sync():
        ...
        # The robot may apply relative-step or workspace limits. Persist the
        # returned command so the dataset matches what the hardware executed.
...
def _record_impl(
    ...
    if cfg.resume:
        ...
        print(
            ...
            f"recording {cfg.dataset.num_episodes} additional episode(s) this run."
        )
    ...
    with (
        ...
        while recorded_episodes < cfg.dataset.num_episodes and not events["stop_recording"]:
            ...
            if teleop is not None and isinstance(teleop, XArmMockTeleop):
            ...
            if is_recorded and not events['stop_recording']:
                if not is_evt:
                    decision = _prompt_headless_episode_decision()
                ...
                if is_evt:
                    ...
                    if _prompt_headless_next_episode() == "quit":
                        events["stop_recording"] = True
                        continue
                    is_recorded = True
    ...
    # Do not leave an enabled arm connected during a potentially long Hub upload.
    disconnect_devices(cleanup_devices, suppress_errors=False)
    cleanup_devices.clear()
...
def main():
    ...
    parser.add_argument('-r', '--resume',
                       action='store_true',
                       default=False,
                       help='Continue recording on an existing dataset (default: False)')
    ...
    parser.add_argument('--check-config-only',
                       action='store_true',
                       default=False,
                       help='Parse and validate the recording config without connecting hardware')
    args, unknown = parser.parse_known_args()
    sys.argv = [sys.argv[0]] + unknown
    register_third_party_plugins()
    cfg = get_cfg()
    if args.check_config_only:
        print("record_config_runtime_parse=PASS")
        return
    if args.resume:
        cfg.resume = True
