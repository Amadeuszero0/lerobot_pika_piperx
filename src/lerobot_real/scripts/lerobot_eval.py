...
def _eval_loop_impl(cfg: EvalConfig, cleanup_devices: list, relative=False, rx_continuous=False):
    init_logging()
...
def eval_loop(cfg: EvalConfig, relative=False, rx_continuous=False):
    ...
    try:
        result = _eval_loop_impl(cfg, cleanup_devices, relative, rx_continuous)
