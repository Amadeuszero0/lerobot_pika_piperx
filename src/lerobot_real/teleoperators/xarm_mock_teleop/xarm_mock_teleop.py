...
from .xarm_mock_teleop_config import XArmMockTeleopConfig
...
class XArmMockTeleop(Teleoperator, threading.Thread):
    
    config_class = XArmMockTeleopConfig
    name = "xArm Mock Teleoperator"

    def __init__(self, config: XArmMockTeleopConfig):
    ...
    def run(self):
        ...
        print(f"Mock teleoperator RT report Thread starts!! size={size}")
        gripper_pos = 1.0
    ...
    def get_action(self) -> dict[str, Any]:
        if not self.is_connected:
            raise DeviceNotConnectedError(
                "XArmMockTeleop is not connected. You need to run `connect()` before `get_action()`."
            )
