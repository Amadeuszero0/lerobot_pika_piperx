...
def make_cameras_from_configs(camera_configs: dict[str, CameraConfig]) -> dict[str, Camera]:
    lerobot_camera_configs = {key: cfg for key, cfg in camera_configs.items() if not cfg.type.startswith("lerobot_real::")}
    real_camera_configs = {key: cfg for key, cfg in camera_configs.items() if cfg.type.startswith("lerobot_real::")}
    cameras = lerobot_make_cameras_from_configs(lerobot_camera_configs)
    
    for key, cfg in real_camera_configs.items():
        if cfg.type == "lerobot_real::umi_camera":
